...
all: gen-dockerfiles library/kengine .github/dependabot.yml
...
gen-dockerfiles: render-dockerfiles.tmpl Dockerfile.tmpl Dockerfile.builder.tmpl Dockerfile.windows.tmpl Dockerfile.windows-builder.tmpl */*/Dockerfile.base
	@gomplate \
		...
		-t kengine-builder=kengine-builder.sh.tmpl \
		-c config=./stackbrew-config.yaml \
		-f $<

library/kengine: stackbrew.tmpl stackbrew-config.yaml gen-dockerfiles
	@gomplate \
